---
name: davinci-sdk
description: Use this skill whenever the user is building, debugging, or asking about code that uses the Vocdoni Davinci SDK (`@vocdoni/davinci-sdk`) — the TypeScript SDK for the Davinci zk-based voting protocol. Triggers on imports from `@vocdoni/davinci-sdk`; mentions of the `DavinciSDK` class, `createProcess` / `submitVote` / `waitForVoteStatus`, the census classes (`OffchainCensus`, `CspCensus`, `OnchainCensus`, `PublishedCensus`), `CensusOrigin`, `VoteStatus` / `TxStatus`, `ProcessRegistryService`, the sequencer or census services, encrypted/homomorphic ballots, zk-SNARK ballot proofs, ballot modes (approval/ranked/quadratic/budget), token-holder / on-chain censuses, or CSP voting. Also load it for high-level intents like "create an election", "let users vote privately", "encrypt a ballot", or "submit a vote to the sequencer" — the SDK has specific class/method shapes (a high-level `DavinciSDK` facade, a two-status async vote flow, and a parametric ballot model) the agent will not recall accurately unaided.
---
